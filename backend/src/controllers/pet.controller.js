...
export const fetchForm = async (request, response) => {
    try {
        ...
        const petsWithMood = pets.map(pet => {
            ...
            return { id: pet._id, name: pet.name, species: pet.species, age: pet.age, personality: pet.personality, mood: newMood, adopted: pet.adopted, adoption_date: pet.adoption_date, }
...
export const formSubmit = async (request, response) => {
    ...
    try {
        ...
        response.status(201).json({ message: 'Pet added successfully', data: newPet });
    } catch (error) {
        console.error(error);
...
export const fetchConMood = async (request, response) => {
    let { mood } = request.body;
    mood = mood.charAt(0).toUpperCase() + mood.slice(1).toLowerCase();
    console.log(mood)

    const findMood = await Pet.find({ mood });
    return response.status(200).json({ success: true, message: 'found', data: findMood })
